/*!
 * FinCalc Smart shared.js v5.0 — 40 calculators, 2026
...
function isBlogPage(){return window.location.pathname.indexOf('/blog/')!==-1;}
function calcBase(){
  if(isSubpage())return '';
  if(isBlogPage())return '../calculators/';
  return 'calculators/';
}
function siteRoot(){
  if(isSubpage())return '../';
...
function buildNav(cb,sr){
  return '<nav class="cw-nav" id="cw-nav">'
    +'<a href="'+sr+'index.html" class="cw-logo">FinCalc Smart<span class="cw-ld">.</span><span class="cw-pro">PRO</span></a>'
    ...
    +'<li><a class="cw-nla" href="'+sr+'blog/index.html" style="border-bottom:2px solid transparent">Financial Guides</a></li>'+'</ul>'
    ...
    +'<a class="cw-ncta" href="'+sr+'all-calculators.html">All Calculators</a>'
...
function buildFooter(cb,sr){
  return '<footer class="cw-ft">'
    +'<div class="cw-ft-top">'
    +'<div class="cw-fb"><div class="cw-flogo">FinCalc Smart<span>.</span></div>'
    ...
    +'<a href="mailto:contact@fincalcsmart.com" class="cw-fso" aria-label="Contact Email"><svg viewBox="0 0 15 15" fill="none"><rect x="1.5" y="3" width="12" height="9" rx="1.5" stroke="rgba(255,255,255,.4)" stroke-width="1.1"/><path d="M1.5 5.5L7.5 9.5L13.5 5.5" stroke="rgba(255,255,255,.4)" stroke-width="1.1" stroke-linecap="round"/></svg></a>'
    +'</div></div>'
    ...
    +'</div>'
    +'<div class="cw-fb2">'
    +'<span class="cw-fcp">© 2026 FinCalc Smart. All rights reserved.</span>'
    +'<div class="cw-fll">'
    ...
    +'<h3>1. Information We Collect</h3><p>FinCalc Smart does not require account registration or collect personal information. To automatically set your region (Canada or USA), we use a free IP geolocation service (ipapi.co) on your first visit. No personal data is stored on our servers. Your region preference is saved locally in your browser (localStorage) so the API is only called once. All calculations are performed locally in your browser. We do not store, transmit, or share your financial inputs.</p>'
    ...
    +'<h3>4. Third-Party Services</h3><p>FinCalc Smart may display contextual affiliate links and advertisements. These third parties have their own privacy policies. We are not responsible for their data practices.</p>'
    ...
    +'<h3>6. Contact</h3><p>For privacy-related questions, please email us at contact@fincalcsmart.com.</p>'
    +'</div>'
    ...
    +'<h3>Not Financial Advice</h3><p>All content, calculators, and results provided by FinCalc Smart are for <strong>informational and educational purposes only</strong>. Nothing constitutes financial, investment, tax, legal, or accounting advice.</p>'
    ...
    +'<h3>Limitation of Liability</h3><p>FinCalc Smart and its operators shall not be liable for any losses or decisions made in reliance on information provided on this website.</p>'
    +'</div>';
}
...
/* initFaqs removed - using onclick=window.toggleFaq */
...
function init(){
  ...
  initNavSearch(cb);initBurger();initCountryToggle();
  detectAndSetCountry();
}

/* ── IP-based Country Detection ─────────────────────────────────────────────
 * Flow: localStorage → ipapi.co API → fallback 'us'
 * Saves to localStorage ONLY after IP detection OR explicit user click.
 * Does NOT save on the hardcoded HTML default to avoid polluting storage. */
function detectAndSetCountry(){
  var KEY = 'fcs-country';

  function applyCountry(mode, save){
    // Toggle button active state
    document.querySelectorAll('.cw-tbtn').forEach(function(btn){
      btn.classList.toggle('on', btn.dataset.mode === mode);
    });
    // data-show / data-ca / data-us visibility
    document.querySelectorAll('[data-show]').forEach(function(el){
      el.style.display = el.dataset.show === mode ? '' : 'none';
    });
    document.querySelectorAll('[data-ca]').forEach(function(el){
    ...
    // Save to localStorage (only when explicitly requested)
    if(save){
      try { localStorage.setItem(KEY, mode); } catch(e){}
    }
  }

  // Attach localStorage save to ALL toggle clicks (user explicit choice)
  document.querySelectorAll('.cw-tbtn').forEach(function(btn){
    btn.addEventListener('click', function(){
      try { localStorage.setItem(KEY, btn.dataset.mode || 'us'); } catch(e){}
    });
  });

  // Step 1: Check localStorage — user's explicit past choice always wins
  var saved = '';
  try { saved = localStorage.getItem(KEY) || ''; } catch(e){}
  if(saved === 'ca' || saved === 'us'){
    applyCountry(saved, false); // already saved — no need to re-save
    return;
  }

  // Step 2: No saved preference — detect via IP (async, non-blocking)
  fetch('https://ipapi.co/json/', {method:'GET', headers:{Accept:'application/json'}})
    .then(function(res){ return res.json(); })
    .then(function(data){
      var cc = (data && data.country_code ? data.country_code : '').toUpperCase();
      var mode = cc === 'CA' ? 'ca' : 'us';
      applyCountry(mode, true); // save the detected country
    })
    .catch(function(){
      // API failed or limit hit — stay on HTML default 'us', don't save
      // Next visit will try the API again
    });
}


/* ── Global comma-safe number parser ────────────────────────────
 * Strips commas and currency symbols before parseFloat.
 * Use: getNum(el.value) instead of parseFloat(el.value)        */
window.getNum = function(v, fallback){
  if(v === null || v === undefined) return fallback !== undefined ? fallback : 0;
  var cleaned = String(v).replace(/[$,\s]/g, '');
  var n = parseFloat(cleaned);
  return (isNaN(n) || !isFinite(n)) ? (fallback !== undefined ? fallback : 0) : n;
};


/* ── Scroll-to-result on mobile — ONLY on explicit Calculate click ──
 * Tracks whether a Calculate button was just clicked by the user.
 * Auto-calc on page load, oninput live updates, and URL-restore
 * do NOT trigger the scroll — only an explicit button tap does.    */
(function(){
  var _userClickedCalc = false;

  // Mark intent when user taps any .calc-btn button
  document.addEventListener('click', function(e){
    var btn = e.target.closest('.calc-btn, [aria-label="Calculate"]');
    if(btn) _userClickedCalc = true;
  }, true); // capture phase — fires before onclick

  function scrollToResults(){
    if(!_userClickedCalc) return;       // ignore auto-calc
    _userClickedCalc = false;           // reset for next interaction
    if(window.innerWidth > 768) return; // desktop — no scroll needed
    var panel = document.querySelector('.calc-results-panel');
    if(!panel) return;
    setTimeout(function(){
      var offset = 72; // sticky nav height
      var top = panel.getBoundingClientRect().top + window.pageYOffset - offset;
      window.scrollTo({top: top, behavior: 'smooth'});
    }, 150);
  }

  // Wrap window.calc and window.c after page scripts define them
  window.addEventListener('load', function(){
    var origCalc = window.calc;
    var origC    = window.c;
    if(typeof origCalc === 'function'){
      window.calc = function(){ origCalc.apply(this, arguments); scrollToResults(); };
    }
    if(typeof origC === 'function'){
      window.c = function(){ origC.apply(this, arguments); scrollToResults(); };
    }
  });
})();
